...
<Callout type="info">
这里的反代配置写的是前后端均通过本地进行部署的情况，若前端通过 Vercel 等平台部署，请忽略/修改对应前端配置部分。
</Callout>
...
此处仅提供前端通过本地部署的情况，若前端通过 Vercel 等平台部署，请忽略/修改对应前端配置部分。
...
- `disable_cache` 是否禁用缓存，默认为 `false`
