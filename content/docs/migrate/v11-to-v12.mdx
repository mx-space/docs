...
### 4. 把 MongoDB 容器接入 v12 网络

迁移脚本要在同一个 docker 网络里同时访问到 mongo 和 postgres。先查询 v12 compose 创建的网络名：
...
把旧 mongo 容器接入这个网络（容器名替换为你 `docker ps` 看到的实际名字）：
...
参数说明：
- `MONGO_URI` 中的 `mongo` 是上一步接入网络的容器名；如果你的容器名不同，对应替换
- `mx-space` 是 v11 默认的数据库名，自定义过的请改
...
| 错误提示 | 原因 | 解决 |
|---------|------|------|
| `Connection refused` (mongo) | 容器名不对 / 未接入网络 | 重做第 4 步，确认容器名 |
| `Connection refused` (postgres) | PostgreSQL 还没启动好 | 等 30 秒重试 |
| `Missing refs > 0` | 有孤儿数据 | 通常可忽略，截图记录后继续 |
| 其他错误 | 未知问题 | **不要继续**，保留日志，回滚到 v11（见下方回滚章节） |

### 6. 断开旧 MongoDB 网络（可选清理）
...
```bash
cd packages/mongo-pg-cli
pnpm build
SNOWFLAKE_WORKER_ID=1 \
MONGO_URI="mongodb://localhost:27017/mx-space" \
PG_URL="postgresql://mx:mx@localhost:5432/mx_core" \
node dist/cli.mjs --mode dry-run
```
...
MONGO_URI="mongodb://localhost:27017/mx-space" \
PG_URL="postgresql://mx:mx@localhost:5432/mx_core" \
MIGRATIONS_DIR="$(pwd)/../../apps/core/src/database/migrations" \
node dist/cli.mjs --mode apply
```
