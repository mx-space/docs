...
1. 在 Core 环境变量中设置 `COMPANION_LIVE_DESK_ENABLED=true`，然后重启 Core。
2. 在后台 **Companion** 页面生成一次性配对码。配对码十分钟后过期，且只能使用一次。
3. 在 Yohaku Companion 的 **Settings → Yohaku** 中填写站点公开 URL、设备名称和配对码。
4. 检查 **Current Sanitized Preview**，再显式启用 Live Desk。完成配对本身不会开始公开状态。
5. 将主题配置中的 `config.module.liveDesk.enable` 设为 `true`。
