...
__webpack_require__.d(__webpack_exports__, {
  createStaticClient: () => (/* binding */ createStaticClient)
});

// EXTERNAL MODULE: ./node_modules/fumadocs-core/dist/chunk-2V6SCS43.js
var chunk_2V6SCS43 = __webpack_require__(7165);
...
function defaults_isGeoPointType(type) {
    return type === 'geopoint';
}
function defaults_isVectorType(type) {
    return typeof type === 'string' && /^vector\[\d+\]$/.test(type);
}
function defaults_isArrayType(type) {
...
function components_insert(implementation, index, prop, id, internalId, value, schemaType, language, tokenizer, docsCount, options) {
    if (defaults_isVectorType(schemaType)) {
    ...
    if (!defaults_isArrayType(schemaType)) {
...
function calculateResultScores(index, prop, term, ids, docsCount, bm25Relevance, resultsMap, boostPerProperty, whereFiltersIDs) {
...
function searchInProperty(index, tree, prop, tokens, exact, tolerance, resultsMap, boostPerProperty, bm25Relevance, docsCount, whereFiltersIDs) {
    ...
    for (let i = 0; i < tokenLength; i++) {
        ...
        for (let j = 0; j < termsFoundLength; j++) {
            ...
            calculateResultScores(index, prop, word, ids, docsCount, bm25Relevance, resultsMap, boostPerProperty, whereFiltersIDs);
        }
    }
}
function components_search(index, term, tokenizer, language, propertiesToSearch, exact, tolerance, boost, relevance, docsCount, whereFiltersIDs) {
    ...
    for (const prop of propertiesToSearch) {
        ...
        searchInProperty(index, tree.node, prop, tokens, exact, tolerance, resultsMap, boostPerProperty, relevance, docsCount, whereFiltersIDs);
    }
...
;// CONCATENATED MODULE: ./node_modules/@orama/orama/dist/browser/methods/docs.js
function getByID(db, id) {
    return db.documentsStore.get(db.data.docs, id);
}
function docs_count(db) {
...
const types_kInsertions = Symbol('orama.insertions');
const types_kRemovals = Symbol('orama.removals');
//# sourceMappingURL=types.js.map
;// CONCATENATED MODULE: ./node_modules/@orama/orama/dist/browser/components/sync-blocking-checker.js
...
function sync_blocking_checker_trackInsertion(orama) {
...
function sync_blocking_checker_trackRemoval(orama) {
...
function insert_insert(orama, doc, language, skipHooks, options) {
    ...
    const asyncNeeded = isAsyncFunction(orama.index.beforeInsert) ||
...
;// CONCATENATED MODULE: ./node_modules/@orama/orama/dist/browser/constants.js
    ...
    for (let i = 0; i < allDocsLength; i++) {
        ...
        for (const facet of facetKeys) {
            ...
            switch (propertyType) {
                ...
                default:
                    throw errors_createError('FACET_NOT_SUPPORTED', propertyType);
            }
        }
    }
...
function getGroups(orama, results, groupBy) {
    ...
    for (let i = 0; i < propertiesLength; i++) {
        ...
        if (typeof schemaProperties[property] === 'undefined') {
            throw errors_createError('UNKNOWN_GROUP_BY_PROPERTY', property);
        }
        if (!ALLOWED_TYPES.includes(schemaProperties[property])) {
            throw errors_createError('INVALID_GROUP_BY_PROPERTY', property, ALLOWED_TYPES.join(', '), schemaProperties[property]);
        }
    }
    const allIDs = results.map(([id]) => internal_document_id_store_getDocumentIdFromInternalId(orama.internalDocumentIDStore, id));
...
function innerFullTextSearch(orama, params, language) {
    ...
    if (properties && properties !== '*') {
        for (const prop of properties) {
            if (!propertiesToSearch.includes(prop)) {
                throw errors_createError('UNKNOWN_INDEX', prop, propertiesToSearch.join(', '));
            }
        }
    ...
    if (term || properties) {
        const docsCount = docs_count(orama);
        ...
        if (params.sortBy) {
            if (typeof params.sortBy === 'function') {
                ...
                uniqueDocsArray = orama.sorter
                    ...
                    .map(([id, score]) => [internal_document_id_store_getInternalDocumentId(orama.internalDocumentIDStore, id), score]);
            }
        }
        else {
...
function innerVectorSearch(orama, params, language) {
    ...
    if (vector && (!('value' in vector) || !('property' in vector))) {
        throw errors_createError('INVALID_VECTOR_INPUT', Object.keys(vector).join(', '));
    }
    const vectorIndex = orama.data.index.vectorIndexes[vector.property];
    const vectorSize = vectorIndex.node.size;
    if (vector?.value.length !== vectorSize) {
        if (vector?.property === undefined || vector?.value.length === undefined) {
            throw errors_createError('INVALID_INPUT_VECTOR', 'undefined', vectorSize, 'undefined');
        }
        throw errors_createError('INVALID_INPUT_VECTOR', vector.property, vectorSize, vector.value.length);
    }
    const index = orama.data.index;
    let whereFiltersIDs;
    const hasFilters = Object.keys(params.where ?? {}).length > 0;
    ...
    function performSearchLogic() {
        ...
        for (let i = 0; i < limit; i++) {
            ...
            if (doc) {
                ...
                const newDoc = {
                    id: internal_document_id_store_getDocumentIdFromInternalId(orama.internalDocumentIDStore, result[0]),
                    score: result[1],
                    document: doc
                };
                docs[i] = newDoc;
...
function search_search(orama, params, language) {
    ...
    throw errors_createError('INVALID_SEARCH_MODE', mode);
}
function fetchDocumentsWithDistinct(orama, uniqueDocsArray, offset, limit, distinctOn) {
    ...
    for (let i = 0; i < uniqueDocsArrayLength; i++) {
        ...
        results.push({ id: internal_document_id_store_getDocumentIdFromInternalId(orama.internalDocumentIDStore, id), score, document: doc });
        resultIDs.add(id);
...
function fetchDocuments(orama, uniqueDocsArray, offset, limit) {
    ...
    for (let i = offset; i < limit + offset; i++) {
        ...
        if (!resultIDs.has(id)) {
            ...
            results[i] = { id: internal_document_id_store_getDocumentIdFromInternalId(orama.internalDocumentIDStore, id), score, document: fullDoc };
            resultIDs.add(id);
        }
    }
    return results;
...
//# sourceMappingURL=index.js.map
;// CONCATENATED MODULE: ./node_modules/fumadocs-core/dist/chunk-2ZSMGYVH.js


// src/search/search/simple.ts

async function searchSimple(db, query, params = {}) {
  const result = await search_search(db, {
    term: query,
    tolerance: 1,
    ...params,
    boost: {
...
async function searchAdvanced(db, query, tag, extraParams = {}) {
  let params = {
    where: (0,chunk_2V6SCS43/* removeUndefined */.o)({
      tag,
      ...extraParams.where
    }),
    groupBy: {
  ...
  const result = await search_search(db, params);
  const list = [];
...
;// CONCATENATED MODULE: ./node_modules/fumadocs-core/dist/static-5GPJ7RUY.js
  ...
  async function init() {
    ...
    if (data.type === "i18n") {
      Object.entries(data.data).forEach(([k, v]) => {
        const db = create_create({ schema: { _: "string" } });
        serialization_load(db, v);
        dbs.set(k, {
          type: v.type,
          db
        });
